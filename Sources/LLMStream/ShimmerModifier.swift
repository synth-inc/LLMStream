...
public struct ShimmerModifier: ViewModifier {
    ...
    public static let defaultAnimation = Animation.linear(duration: 1.5).delay(0.25).repeatForever(
        autoreverses: false)
    ...
    @ViewBuilder public func applyingGradient(to content: Content) -> some View {
        let gradient = LinearGradient(
            gradient: gradient, startPoint: startPoint, endPoint: endPoint)
        switch mode {
        case .mask:
...
extension View {
    ...
    @ViewBuilder public func shimmering(
        ...
        if active {
            modifier(
                ShimmerModifier(
                    animation: animation, gradient: gradient, bandSize: bandSize, mode: mode))
        } else {
            self
        }
    }
    ...
    @available(
        *, deprecated, message: "Use shimmering(active:animation:gradient:bandSize:) instead."
    )
    @ViewBuilder public func shimmering(
